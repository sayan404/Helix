...
export async function POST(request: NextRequest) {
  return monitorApiRoute(
    { route: "/api/export-boilerplate", method: "POST", request },
    async () => {
      try {
        ...
        if (archId) {
          ...
          const arch = await db
            ...
            .where(
              and(
                eq(schema.architectures.id, archId),
        ...
        if (!templates.length) {
          return new Response(
            JSON.stringify({ error: "No code templates to export" }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" },
            }
        ...
        return new Response(
          JSON.stringify({ error: "Failed to export boilerplate" }),
          {
            status: 500,
            headers: { "Content-Type": "application/json" },
          }
